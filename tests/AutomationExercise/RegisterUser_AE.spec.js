...
test('login_signup_AE', async ({ page }) => {
   ...
   const signupForm = page.locator('//h2[text()="New User Signup!"]');
  ...
  let emailaddress = "Sunitha"+x+"@test.com"

  // Entering Address information. 

  const loginForm = page.locator('//b[text()="Enter Account Information"]');// using class name
  ...
  const deleteAccountLink = page.getByRole('link', { name: 'Delete Account' });
  const logoutLink = page.getByRole('link', { name: 'logout'});
  const accountDeleted = page.locator('//b[text()="Account Deleted!"]');
  const deleteContinueButton = page.locator('div[class="pull-right"]');
  ...
  // ACTUAL STEPS STARING FROM HERE //
    ...
    await expect(signupForm).toHaveText("New User Signup!");
   ...
   await expect(loginForm).toHaveText('Enter Account Information');
...
// clicking on Delete link and then clicking on continue button of deletedaccount link
//await deleteAccountLink.click();
//await expect(accountDeleted).toHaveText('Account Deleted!');
//await deleteContinueButton.click();

// clicking on logout link and then clicking on signup/login link. 
await logoutLink.click();
await signupLoginLink.click();
// Asserting the url, after tapping on login link
await expect(page).toHaveURL("https://www.automationexercise.com/login")
// Entering same email address in login form.
await loginEmail.fill(emailaddress);
